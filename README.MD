...
- Supported field types:
  ...
  - ColorInput (Color picker with multiple formats and swatches)
...
### CSS Imports (Mantine v8)

Bu kütüphane Mantine v8 kullanıyor. Uygulamanızda gerekli CSS dosyalarını import etmelisiniz:

```tsx
// Ana uygulama dosyanızda (App.tsx, _app.tsx, layout.tsx vb.)
...
**Not:** Mantine v8'de her paket için CSS import'u manuel olarak yapılmalıdır.
...
- defaultValue: Default value for the field (string | number) - applicable to most field types

### Default Values

Most field types support the `defaultValue` property to set an initial value when no `initialData` is provided:

**Supported Field Types:**
- **Textbox/Textarea/MaskInput**: `defaultValue: string` - Sets default text
- **Number**: `defaultValue: number` - Sets default numeric value
- **Checkbox/Switch**: `defaultValue: boolean` - Sets default checked state (Note: Switch also supports `defaultChecked`)
- **Dropdown**: `defaultValue: string | number` - Sets default selected option value
- **SegmentedControl**: `defaultValue: string | number` - Sets default selected segment
- **Date/DateTime**: Use `initialData` instead, `defaultValue` not applicable

**Important Notes:**
- `defaultValue` is only used when `initialData[field.field]` is undefined
- For Dropdown and SegmentedControl, the component uses Mantine's native `defaultValue` prop
- The value type must match the field type (string for text, number for numeric, boolean for checkbox)
...
```tsx
{
  ...
  title: "Country",
  type: "dropdown",
  defaultValue: "US", // Will select "US" option by default
  options: [
...
```tsx
{
  ...
  maxLength: 100,
  minLength: 2,
  defaultValue: "John Doe" // Default text
}
...
#### Checkbox
- type: "checkbox"
- defaultValue: Default checked state (boolean)
...
- dropdownBackgroundColor: Dropdown menu background color (default: '#f5f5f5')
- selectFirst: Auto select first option (default: false)
...
```tsx
{
  ...
  defaultValue: "TR", // Default selected value
  dropdownBackgroundColor: "#ffffff", // Custom dropdown menu background
  selectFirst: true // Auto select first option (default: false)
}

// Enable auto-select first option (must be explicitly set)
{
  field: "city",
  title: "City",
  type: "dropdown",
  ...
  // selectFirst not set, so first option won't be selected (default: false)
}
```
...
#### Tree
- type: "tree"
- optionsUrl: API URL for hierarchical tree data
- options: Static hierarchical tree data
- is_dropdown: Boolean for dropdown mode (default: false)
- levelOffset: Tree level offset for indentation

```tsx
// Tree selection (checkbox mode)
{
  field: "categories",
  title: "Categories",
  type: "tree",
  required: true,
  optionsUrl: "https://api.example.com/tree-categories",
  is_dropdown: false
}

// Dropdown mode (single selection)
{
  field: "category",
  title: "Category",
  type: "tree",
  ...
  options: [
    {
      ...
      label: "Engineering",
      children: [
        { value: "1.1", label: "Frontend" },
        { value: "1.2", label: "Backend" }
      ]
    },
    {
      value: "2",
...
Features of Tree:
- Hierarchical data selection with expandable/collapsible nodes
- Two modes: checkbox (multi-select) and dropdown (single-select)
- Support for nested children at any depth
- Dynamic loading from API or static data
...
#### ColorInput
- type: "colorinput"
- format: Color format - 'hex' | 'hexa' | 'rgb' | 'rgba' | 'hsl' | 'hsla' (default: 'hex')
- defaultValue: Default color value (string)
- swatches: Array of predefined color swatches
- swatchesPerRow: Number of swatches per row (default: 7)
- withPicker: Show/hide color picker dropdown (default: true)
- withEyeDropper: Show/hide eye dropper tool (default: true)
- closeOnColorSwatchClick: Close dropdown when swatch is clicked (default: false)
...
// Without picker, only swatches
{
  ...
  swatches: ['#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A'],
  closeOnColorSwatchClick: true
}
```

Features of ColorInput:
- Multiple color format support (hex, hexa, rgb, rgba, hsl, hsla)
- Eye dropper tool for picking colors from screen (browser support required)
- Predefined color swatches for quick selection
- Customizable swatches per row
- Alpha/opacity slider for hexa, rgba, and hsla formats
- Color picker with hue and saturation controls
- Can be used with or without the picker dropdown
- Form validation integration
- Default value support

**Supported Color Formats:**
- `hex`: #RRGGBB (e.g., #228BE6)
- `hexa`: #RRGGBBAA (e.g., #228BE680) - includes alpha/opacity slider
- `rgb`: rgb(R, G, B) (e.g., rgb(34, 139, 230))
- `rgba`: rgba(R, G, B, A) (e.g., rgba(34, 139, 230, 0.5)) - includes alpha/opacity slider
- `hsl`: hsl(H, S%, L%) (e.g., hsl(211, 79%, 52%))
- `hsla`: hsla(H, S%, L%, A) (e.g., hsla(211, 79%, 52%, 0.5)) - includes alpha/opacity slider
